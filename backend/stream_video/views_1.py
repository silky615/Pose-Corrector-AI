...
def get_exercise_detection():
    """Lazy-load exercise_detection function."""
    try:
        from detection.main import exercise_detection
        return exercise_detection
    except Exception as e:
        print(f"Warning: Could not import exercise_detection: {e}")
        return None
...
def _reset_exercise_counter(client_key: str, ex_type: str) -> None:
    ...
    if ex_type == "bicep_curl":
        ...
        _TREE_POSE_RT_STATE[client_key] = {"hold_start_ts": None, "counter": 0, "rep_counted": False}
...
_TREE_POSE_RT_STATE = {}
TREE_POSE_HOLD_SECONDS = 3  # seconds of good hold required = 1 rep

def _ensure_tree_pose_state(client_key: str):
    if client_key not in _TREE_POSE_RT_STATE:
        _TREE_POSE_RT_STATE[client_key] = {
            "hold_start_ts": None,   # timestamp when current hold started
            "counter": 0,            # completed reps
            "rep_counted": False,    # prevent double-counting same hold
        }
    ...
    try:
        ...
        if ex_type == "tree_pose":
            ...
            if not foot_lifted or not lifted_leg_bent:
                _TREE_POSE_RT_STATE[client_key]["hold_start_ts"] = None
                _TREE_POSE_RT_STATE[client_key]["rep_counted"] = False
                return Response(
                    {
                        "message": f"Tree Pose: Lift your {lifted_side.lower()} foot and place it on the inner leg of the standing leg.",
                        "accuracy": 0,
                        "posture_ok": False,
                        "counter": _TREE_POSE_RT_STATE[client_key]["counter"],
                    }
                )
            ...
            # All checks passed — good pose, start/continue hold timer
            now_ts = datetime.now().timestamp()
            st = _TREE_POSE_RT_STATE[client_key]

            if st["hold_start_ts"] is None:
                st["hold_start_ts"] = now_ts
                st["rep_counted"] = False  # fresh hold, allow counting once

            elapsed = now_ts - float(st["hold_start_ts"])
            remaining = max(0.0, TREE_POSE_HOLD_SECONDS - elapsed)

            # Count rep once per completed hold
            if elapsed >= TREE_POSE_HOLD_SECONDS and not st["rep_counted"]:
                st["counter"] += 1
                st["rep_counted"] = True

            if remaining > 0:
                msg = f"Tree Pose: Great balance! Hold steady... {remaining:.1f}s remaining"
                acc = int(min(99, 70 + (elapsed / TREE_POSE_HOLD_SECONDS) * 30))
            else:
